...
console.log(`
🌙 | ${chalk.green('user :')} ${chalk.white('%s')}
🌙 | ${chalk.redBright('type : ❱❱')} ${chalk.greenBright.bold('[%s]')}
`.trim(),          
...
m ? '' : '',
`.`,
(m.chat.includes("@g.us") ? `${chalk.green('group :')} ${chalk.yellowBright.italic(`${m.chat} ~${chat}`)}` : `${chalk.green('chat :')} ${chalk.yellowBright.italic(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
